...
def process_downstream_action(db: Session, request_id: str, action: str):
    """
    ...
    if db_request.prediction_id:
        schedule = db.query(ServiceSchedule).filter(ServiceSchedule.prediction_id == db_request.prediction_id).first()
        if schedule:
...
def handle_notification_response(
    ...
    db_request = db.query(ServiceRequest).filter(
        ServiceRequest.prediction_id == prediction_id
    ).first()
    ...
    # Or, we allow changing the decision if it hasn't been executed yet? Let's assume once approved/ignored, it's final
    ...
    if action == "APPROVE_SERVICE":
        ...
        # In a real app, we might assign a real scheduled_time here
